...
@app.get("/")
def read_root():
    return {"message": "RailSathi API is running!"}

@app.get("/health")
def health_check():
    return {"status": "RailSathi is healthy!"}
...
def submit_complaint(complaint: Complaint):
    # You can add logic to store this in DB or send email
    return {
        "status": "success",
        "message": f"Complaint received from {complaint.name}",
        "data": complaint.dict()
    }
